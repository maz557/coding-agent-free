...
**v1.10 میں نیا — ویب UI میں بہتری:**
- **Diff Viewer** — فائل لکھنے، تبدیل کرنے یا شامل کرنے پر سطر بہ سطر فرق (سبز + / سرخ -) دکھائیں۔
- **سیشن مینیجر** — سیشن بنائیں، ان کے درمیان سوئچ کریں، پہلے پیغام سے خودکار عنوان، ماڈل اور پیغامات کی تعداد دکھائیں۔
- **Slash کمانڈز** — `/active`، `/model 2`، `/safe`، `/allow`، `/reset`، `/models`، `/exit` ویب انپٹ میں۔
- **ہیلپ موڈل** — `?` بٹن کے ساتھ استعمال گائیڈ، ماڈل سوئچنگ، کمانڈز ریفرنس اور Diff Viewer کی وضاحت۔
- **SSE اسٹریمنگ** — `fetch` + `ReadableStream` کا استعمال (`EventSource` پر انحصار نہیں)۔
...
> 📝 ٹیسٹ چلائیں: `npm run test:unit` (137 یونٹ ٹیسٹ) — `npm run test:integration` (26 پرووائیڈر انٹیگریشن ٹیسٹ) — `npm test` (35 انٹیگریشن ٹیسٹ)
